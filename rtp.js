...
const NTP_SCALE_FRAC = 4294.967295;

// Minimum length of an RTP header
const RTP_HEADER_LEN = 12;

const MAX_PAYLOAD_SIZE = 1360;
      ...
      if (buffers.length >= 2) {
        ...
        return (() => {
          ...
          while (buffers.length >= 2) {
            ...
            const firstPacket = buffers.shift();
            ...
            if (packetBuffer.nextSequenceNumber > MAX_SEQUENCE_NUMBER) {
              result.push(packetBuffer.nextSequenceNumber = 0);
            } else {
...
var api = {
  ...
  readRTCPSenderReport(bits) {
    ...
    // skip padding bytes
    const readBytes = bits.current_position().byte - startBytePos;
        ...
        if (sdesItem.type === 0) {  // terminate the list
          ...
          if (bytesPastBoundary > 0) {
            while (bytesPastBoundary < 4) {
              const nullOctet = bits.read_byte();
              if (nullOctet !== 0x00) {
  ...
  readRTCPGoodbye(bits) {
    ...
    if (bits.has_more_data()) {
      ...
      const reason = bits.read_bytes(info.reasonOctetCount);
    }
  ...
  readH264FragmentationUnitHeader(bits) {
    ...
    const reservedBit = bits.read_bit();
    if (reservedBit !== 0) {
  ...
  ntpTimestampToTime(ntpTimestamp) {
    const sec = ntpTimestamp[0] - EPOCH;
    const ms = ntpTimestamp[1] / NTP_SCALE_FRAC / 1000;
    return (sec * 1000) + ms;
  },
  ...
  getNTPTimestamp(time) {
    ...
    const ntp_sec = sec + EPOCH;
    const ntp_usec = Math.round(ms * 1000 * NTP_SCALE_FRAC);
  ...
  createRTPHeader(opts) {
    const seqNum = opts.sequenceNumber;
    const ts = opts.timestamp;
    const ssrc = opts.ssrc != null ? opts.ssrc : 0;
    return [
      // version(2): 2
      // padding(1): 0
      // extension(1): 0
  ...
  createGoodbye(opts) {
    ...
    const length = ((4 + (ssrcs.length * 4) + 1 + reasonLen + padLen) / 4) - 1;
