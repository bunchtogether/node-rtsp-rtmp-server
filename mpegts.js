...
const SETTIMEOUT_ADVANCE_TIME = 20;
...
let audioPID = null;
let videoPID = null;
const pendingVideoPesPackets = [];
const pendingAudioPesPackets = [];
...
var api = {
  ...
  consumeVideo(doNotReadNext) {
    ...
    const pendingLen = pendingVideoPesPackets.length;
    if (pendingLen > 0) {
  ...
  consumeAudio(doNotReadNext) {
    if (doNotReadNext == null) { doNotReadNext = false; }
    const pendingLen = pendingAudioPesPackets.length;
    if (pendingLen > 0) {
  ...
  read_service_description_section(bits) {
    let CRC_32;
    const pointer_field = bits.read_byte();
    ...
    const section_length = bits.read_bits(12);
    if (section_length > 0x3ff) {
      throw new Error(`The first two bits of section_length must be 00: ${section_length}`);
    }
    const transport_stream_id = bits.read_bits(16);
    reserved = bits.read_bits(2);
    const version_number = bits.read_bits(5);
    const current_next_indicator = bits.read_bit();
    const section_number = bits.read_byte();
    const last_section_number = bits.read_byte();
    ...
    while (remaining_section_length > 0) {
      ...
      const EIT_schedule_flag = bits.read_bit();
      const EIT_present_following_flag = bits.read_bit();
      const running_status = bits.read_bits(3);
      const free_CA_mode = bits.read_bit();
      const descriptors_loop_length = bits.read_bits(12);
      let remaining_descriptors_loop_length = descriptors_loop_length;
      const descriptors = [];
      while (remaining_descriptors_loop_length > 0) {
        const descriptor = api.read_descriptor(bits);
  ...
  read_CA_descriptor(bits) {
    ...
    const reserved = bits.read_bits(3);
    info.CA_PID = bits.read_bits(13);
    const remainingLen = info.descriptor_length - 4;
  ...
  read_program_map_section(bits) {
    ...
    while (remaining_section_length > 0) {
      ...
      reserved = bits.read_bits(3);
      const elementary_PID = bits.read_bits(13);
      switch (stream_type) {
      ...
      reserved = bits.read_bits(4);
      const ES_info_length = bits.read_bits(12);
    ...
    const CRC_32 = bits.read_bits(32);
  ...
  parsePESPacket(pid, packet, opts) {
    ...
    const pes_data = Buffer.concat(packet.data);
  ...
  read_system_header(bits) {
    ...
    const reserved_bits = bits.read_bits(7);
    info.total_length = 12;
    info.bufferBounds = {};
    while (((nextByte = bits.read_byte()) & 0x80) === 0x80) {
      const stream_id = nextByte;
      const bits_11 = bits.read_bits(2);
      if (bits_11 !== 3) {
        throw new Error(`bits must be '11': ${bits_11}`);
      }
      const P_STD_buffer_bound_scale = bits.read_bit();
  ...
  read_pack_header(bits) {
    ...
    marker_bit = bits.read_bit();
    marker_bit = bits.read_bit();
    const reserved = bits.read_bits(5);
  ...
  read_pes_packet(bits, opts) {
    ...
    if (![
      ...
      if (info.PTS_DTS_flags === 2) {
        const bits_0010 = bits.read_bits(4);
        if (bits_0010 !== 2) {
      ...
      if (info.PTS_DTS_flags === 3) {
        const bits_0011 = bits.read_bits(4);
        if (bits_0011 !== 3) {
        ...
        info.PTS += bits.read_bits(15);
        marker_bit = bits.read_bit();
        const bits_0001 = bits.read_bits(4);
        if (bits_0001 !== 1) {
  ...
  read_adaptation_field(bits) {
    ...
    if (info.adaptation_field_length > 0) {
      ...
      if (info.transport_private_data_flag === 1) {
        const transport_private_data_length = bits.read_byte();
        consumed_bytes++;
    ...
    if (!isEOF) {
      while (true) {
        try {
          ...
          const ts_packet = api.read_transport_packet(bits);
