...
var api = {
  ...
  splitIntoNALUnits(buffer) {
    ...
    while (true) {
      var nalUnit;
      const startCodePos = Bits.searchBytesInArray(buffer, [0x00, 0x00, 0x01], 0);
  ...
  read_user_data_unregistered(bits, payloadSize) {
    ...
    const uuid_iso_iec_11578 = bits.read_bits(128);
  ...
  read_sei_message(bits) {
    ...
    const last_payload_type_byte = byte;
    payloadType += last_payload_type_byte;
    let payloadSize = 0;
    while ((byte = bits.read_byte()) === 0xff) {
      payloadSize += 255;
    }
    const last_payload_size_byte = byte;
  ...
  readPPS(nalUnit) {
    ...
    const bits = new Bits(nalUnit);
    api.read_nal_header(bits);
    const pic_parameter_set_id = bits.read_ue();
    const seq_parameter_set_id = bits.read_ue();
    ...
    const pic_init_qs_minus26 = bits.read_se();
    const chroma_qp_index_offset = bits.read_se();
    pps.deblocking_filter_control_present_flag = bits.read_bit();
    const constrained_intra_pred_flag = bits.read_bit();
    ...
    if (api.more_rbsp_data(bits)) {
      ...
      if (pic_scaling_matrix_present_flag === 1) {
        ...
        for (i = 0, end3 = 6 + ((sps.chroma_format_idc !== 3 ? 2 : 6) * transform_8x8_mode_flag), asc3 = end3 >= 0; asc3 ? i < end3 : i > end3; asc3 ? i++ : i--) {
          ...
          if (pic_scaling_list_present_flag) {
            if (i < 6) {
              const scalingList4x4 = [];
              const useDefaultScalingMatrix4x4Flag = [];
              api.readScalingList(bits, scalingList4x4, 16,
                useDefaultScalingMatrix4x4Flag);
            } else {
              const scalingList8x8 = [];
              const useDefaultScalingMatrix8x8Flag = [];
      ...
      const second_chroma_qp_index_offset = bits.read_se();
    }
    // rbsp_trailing_bits()
  },
  ...
  readSPS(nalUnit) {
    ...
    const reserved_zero_2bits = bits.read_bits(2);
    ...
    if ([100, 110, 122, 244, 44, 83, 86, 118, 128].includes(sps.profile_idc)) {
      ...
      const bit_depth_luma_minus8 = bits.read_ue();
      const bit_depth_chroma_minus8 = bits.read_ue();
    ...
    const gaps_in_frame_num_value_allowed_flag = bits.read_bit();
  ...
  read_ref_pic_list_modification(bits, opts) {
    ...
    if (![2, 4].includes(sliceHeader.slice_type % 5)) {
      const ref_pic_list_modification_flag_l0 = bits.read_bit();
    ...
    if ((sliceHeader.slice_type % 5) === 1) {
      const ref_pic_list_modification_flag_l1 = bits.read_bit();
  ...
  read_pred_weight_table(bits, opts) {
    ...
    if (sps.chromaArrayType !== 0) {
      const chroma_log2_weight_denom = bits.read_ue();
    }
    const luma_weight_10 = [];
    ...
    for (i = 0, end = sliceHeader.num_ref_idx_l0_active_minus1, asc = end >= 0; asc ? i <= end : i >= end; asc ? i++ : i--) {
      ...
      if (sps.chromaArrayType !== 0) {
        const chroma_weight_10_flag = bits.read_bit();
    ...
    if ((sliceHeader.slice_type % 5) === 1) {
      const luma_weight_11 = [];
      const luma_offset_11 = [];
      ...
      return (() => {
        ...
        for (i = 0, end1 = sliceHeader.num_ref_idx_l1_active_minus1, asc1 = end1 >= 0; asc1 ? i <= end1 : i >= end1; asc1 ? i++ : i--) {
          ...
          if (sps.chromaArrayType !== 0) {
            const chroma_weight_11_flag = bits.read_bit();
  ...
  read_slice_header(bits, opts) {
    const sliceHeader = (opts.sliceHeader = {});
    ...
    if (sps.separate_colour_plane_flag === 1) {
      const colour_plane_id = bits.read_bits(2);
    }
    ...
    if (pps.redundant_pic_cnt_present_flag) {
      const redundant_pic_cnt = bits.read_ue();
    }
    const sliceTypeString = api.getSliceTypeString(sliceHeader.slice_type);
    if (sliceTypeString === 'B') {
      const direct_spatial_mv_pred_flag = bits.read_bit();
    }
    ...
    if (pps.entropy_coding_mode_flag && (!['I', 'SI'].includes(sliceTypeString))) {
      const cabac_init_idc = bits.read_ue();
    }
    ...
    if (['SP', 'SI'].includes(sliceTypeString)) {
      ...
      const slice_qs_delta = bits.read_se();
    }
    if (pps.deblocking_filter_control_present_flag) {
      const disable_deblocking_filter_idc = bits.read_ue();
      if (disable_deblocking_filter_idc !== 1) {
        const slice_alpha_c0_offset_div2 = bits.read_se();
  ...
  isSamePicture(nalUnit1, nalUnit2) {
    const nalData1 = api.parseNALUnit(nalUnit1);
    const nalData2 = api.parseNALUnit(nalUnit2);
  ...
  removeEmulationPreventionByte(nalUnit) {
    let searchPos = 0;
    const removeBytePositions = [];
    while (true) {
      const emulPos = Bits.searchBytesInArray(nalUnit, [0x00, 0x00, 0x03], searchPos);
      if (emulPos === -1) {
        break;
      }
    ...
    if (removeBytePositions.length > 0) {
      const newBuf = new Buffer(nalUnit.length - removeBytePositions.length);
      let currentSrcPos = 0;
      for (let i = 0; i < removeBytePositions.length; i++) {
        // srcBuf.copy(destBuf, destStart, srcStart, srcEnd)
        const pos = removeBytePositions[i];
  ...
  deriveCtxIdx(input) {
    // input
    const { binIdx } = input;
    const { maxBinIdxCtx } = input;
    const { ctxIdxOffset } = input;
  ...
  // 9.3.3.2.1
  decodeDecision(vars) {
    const qCodIRangeIdx = (vars.codIRange >> 6) & 3;
    const codIRangeLPS = rangeTabLPS[vars.pStateIdx][qCodIRangeIdx];
  ...
  read_hrd_parameters(bits) {
    ...
    const cpb_cnt_minus1 = bits.read_ue();
    const bit_rate_scale = bits.read_bits(4);
    const cpb_size_scale = bits.read_bits(4);
    ...
    const initial_cpb_removal_delay_length_minus1 = bits.read_bits(5);
    const cpb_removal_delay_length_minus1 = bits.read_bits(5);
    const dpb_output_delay_length_minus1 = bits.read_bits(5);
