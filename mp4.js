...
class MP4File extends EventEmitterModule {
  ...
  getSPS() {
    const avcCBox = this.videoTrakBox.find('avcC');
  ...
  getAudioSpecificConfig() {
    const esdsBox = this.audioTrakBox.find('esds');
  ...
  sendVideoPacketsSinceLastKeyFrame(endSeconds, callback) {
    ...
    const stblBox = this.videoTrakBox.child('mdia').child('minf').child('stbl');
    const sttsBox = stblBox.child('stts'); // TimeToSampleBox
    const videoSample = sttsBox.getSampleAfterSeconds(endSeconds);
    if (videoSample != null) {
    ...
    while (true) {
      const rawSample = this.getSample(videoSampleNumber, this.videoTrakBox);
  ...
  seek(seekSeconds) {
    ...
    if (this.videoTrakBox != null) {
      ...
      const videoSample = sttsBox.getSampleAfterSeconds(seekSeconds);
      if (videoSample != null) {
    ...
    if (this.audioTrakBox != null) {
      ...
      sttsBox = stblBox.child('stts'); // TimeToSampleBox
      const audioSample = sttsBox.getSampleAfterSeconds(seekSeconds);
      if (audioSample != null) {
        let minTime;
  ...
  checkAudioBuffer() {
    const timeDiff = this.bufferedAudioTime - this.currentPlayTime;
  ...
  checkVideoBuffer() {
    const timeDiff = this.bufferedVideoTime - this.currentPlayTime;
    if (timeDiff < READ_BUFFER_TIME) {
      // Fill video buffer
  ...
  queueBufferedSamples() {
    ...
    const firstAudioTime = this.bufferedAudioSamples[this.queuedAudioSampleIndex] != null ? this.bufferedAudioSamples[this.queuedAudioSampleIndex].time : undefined;
    const firstVideoTime = this.bufferedVideoSamples[this.queuedVideoSampleIndex] != null ? this.bufferedVideoSamples[this.queuedVideoSampleIndex].time : undefined;
  ...
  getLastTimestamp() {
    ...
    if (this.audioTrakBox != null) {
      const numAudioSamples = this.getNumAudioSamples();
  ...
  getDurationSeconds() {
    const mvhdBox = this.moovBox.child('mvhd');
  ...
  parseH264Sample(buf) {
    ...
    const avcCBox = this.videoTrakBox.find('avcC');
    const lengthSize = avcCBox.lengthSizeMinusOne + 1;
    const bits = new Bits(buf);

    const nalUnits = [];

    while (bits.has_more_data()) {
      const length = bits.read_bits(lengthSize * 8);
  ...
  getSample(sampleNumber, trakBox) {
    const stblBox = trakBox.child('mdia').child('minf').child('stbl');
    const sttsBox = stblBox.child('stts');
    const stscBox = stblBox.child('stsc');
    const chunkNumber = stscBox.findChunk(sampleNumber);

    // Get chunk offset in the file
    const stcoBox = stblBox.child('stco');
    const chunkOffset = stcoBox.getChunkOffset(chunkNumber);

    const firstSampleNumberInChunk = stscBox.getFirstSampleNumberInChunk(chunkNumber);

    // Get an array of sample sizes in this chunk
    const stszBox = stblBox.child('stsz');
    const sampleSizes = stszBox.getSampleSizes(firstSampleNumberInChunk,
      (sampleNumber - firstSampleNumberInChunk) + 1);

    const cttsBox = stblBox.child('ctts');
    const samples = [];
    let sampleOffset = 0;
    ...
    for (let i = 0; i < sampleSizes.length; i++) {
      ...
      if ((firstSampleNumberInChunk + i) === sampleNumber) {
        ...
        const sampleTime = sttsBox.getDecodingTime(sampleNumber);
        const compositionTime = sampleTime.time + compositionTimeOffset;
  ...
  readChunk(chunkNumber, fromSampleNumber, trakBox) {
    ...
    const sttsBox = stblBox.child('stts');
    const stscBox = stblBox.child('stsc');
    const numSamplesInChunk = stscBox.getNumSamplesInChunk(chunkNumber);
    ...
    // Get an array of sample sizes in this chunk
    const stszBox = stblBox.child('stsz');
    const sampleSizes = stszBox.getSampleSizes(firstSampleNumberInChunk, numSamplesInChunk);
    ...
    for (let i = 0; i < sampleSizes.length; i++) {
      ...
      if ((firstSampleNumberInChunk + i) >= fromSampleNumber) {
        ...
        const sampleTime = sttsBox.getDecodingTime(firstSampleNumberInChunk + i);
        const compositionTime = sampleTime.time + compositionTimeOffset;
        if (mdhdBox.timescale !== 90000) {
          pts = Math.floor((compositionTime * 90000) / mdhdBox.timescale);
          dts = Math.floor((sampleTime.time * 90000) / mdhdBox.timescale);
  ...
  readNextVideoChunk() {
    ...
    if ((this.consumedVideoChunks === 0) && (this.consumedVideoSamples !== 0)) { // seeked
      const stscBox = this.videoTrakBox.find('stsc');
      const chunkNumber = stscBox.findChunk(this.consumedVideoSamples + 1);
  ...
  readNextAudioChunk() {
    ...
    if ((this.consumedAudioChunks === 0) && (this.consumedAudioSamples !== 0)) { // seeked
      const stscBox = this.audioTrakBox.find('stsc');
      const chunkNumber = stscBox.findChunk(this.consumedAudioSamples + 1);
...
class Box {
  ...
  dump(depth, detailLevel) {
    ...
    str += `${this.typeStr}`;
    if (detailLevel > 0) {
      const detailString = this.getDetails(detailLevel);
...
class MovieHeaderBox extends Box {
  read(buf) {
    ...
    const reservedInt1 = bits.read_int(32);
    if (reservedInt1 !== 0) {
      throw new Error(`reserved int(32) (1) is not zero: ${reservedInt1}`);
    }
    const reservedInt2 = bits.read_int(32);
    if (reservedInt2 !== 0) {
...
class EditListBox extends Box {
  read(buf) {
    ...
    //     edts
    //       elst <- self
    const mvhdBox = this.findParent('moov').find('mvhd');
    ...
    for (let i = 1, end = entryCount, asc = end >= 1; asc ? i <= end : i >= end; asc ? i++ : i--) {
      ...
      const mediaRateInteger = bits.read_int(16);
      const mediaRateFraction = bits.read_int(16);
  ...
  getEmptyDuration() {
    ...
    for (const entry of Array.from(this.entries)) {
      if (entry.mediaTime === -1) { // empty edit
        ...
        //     edts
        //       elst <- self
        const mvhdBox = this.findParent('moov').child('mvhd');
...
class HandlerBox extends Box {
  read(buf) {
    ...
    const pre_defined = bits.read_bits(32);
    if (pre_defined !== 0) {
...
class SampleDescriptionBox extends Box {
  read(buf) {
    ...
    //           stsd <- self
    //             stts
    const handlerRefBox = this.findParent('mdia').find('hdlr');
    const { handlerType } = handlerRefBox;
...
class HintSampleEntry extends Box {
  read(buf) {
    const bits = new Bits(buf);
    // SampleEntry
    const reserved = bits.read_bits(8 * 6);
    if (reserved !== 0) {
...
class TimeToSampleBox extends Box {
  read(buf) {
    ...
    for (let i = 0, end = this.entryCount, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
      ...
      const sampleDelta = bits.read_uint32();
      if (sampleDelta < 0) {
  ...
  getSampleAfterSeconds(sec, isExclusive) {
    ...
    for (const entry of Array.from(this.entries)) {
      ...
      const entryDuration = entry.sampleDelta * entry.sampleCount;
...
class ESDBox extends Box {
  ...
  readSLConfigDescriptor(bits) {
    ...
    if (info.predefined === 0) {
      ...
      const reserved = bits.read_bits(2);
      if (reserved !== 0b11) {
