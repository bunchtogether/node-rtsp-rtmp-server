...
const rtmp_handshake = require('./rtmp_handshake');
...
const DEBUG_OUTGOING_RTMP_PACKETS = false;

const RTMPT_SEND_REQUEST_BUFFER_SIZE = 10;
...
const parseAMF0StrictArray = function (buf) {
  ...
  let readLen = 4;
  while (--len >= 0) {
    const result = parseAMF0Data(buf.slice(readLen));
...
const parseAMF0DataMessage = function (buf) {
  ...
  while (remainingLen > 0) {
    const result = parseAMF0Data(buf);
...
var createAMF0StrictArray = function (arr, buf = null) {
  ...
  for (let i = 0; i < arr.length; i++) {
    const value = arr[i];
    const valueBytes = createAMF0Data(value);
...
var createAMF0PropertyList = function (obj, buf = null) {
  ...
  for (const name in obj) {
    ...
    const nameLenBytes = new Buffer(2);
    nameLenBytes[0] = (nameLen >> 8) & 0xff;
    nameLenBytes[1] = nameLen & 0xff;
    const dataBytes = createAMF0Data(value);
...
var flushRTMPMessages = function (stream, params) {
  ...
  const rtmpMessagesToSend = queuedRTMPMessages[stream.id];
      ...
      if (session.isResuming) {
        ...
        for (i = 0; i < msgs.length; i++) {
          ...
          if ((rtmpMessage.avType == null)) {
            ...
            filteredMsgs.push(...Array.from(msgs.slice(i) || []));
...
var createRTMPAggregateMessage = function (rtmpMessages, chunkSize) {
  ...
  for (const rtmpMessage of Array.from(rtmpMessages)) {
    ...
    const header = createMessageHeader(rtmpMessage);
    const len = header.length + rtmpMessage.body.length;
  ...
  const aggregateBody = Buffer.concat(bufs, totalLength);
...
var createRTMPMessage = function (params, chunkSize) {
  ...
  if (bodyLength > chunkSize) {
    ...
    while (true) {
      ...
      const bodyChunkLen = bodyChunk.length;
...
class RTMPSession {
  ...
  parseVideoMessage(buf) {
    ...
    switch (info.videoDataTag.avcPacketType) {
      case flv.AVC_PACKET_TYPE_SEQUENCE_HEADER:
        ...
        var sps = h264.concatWithStartCodePrefix(this.avcInfo.sps);
        var pps = h264.concatWithStartCodePrefix(this.avcInfo.pps);
  ...
  createAVMessage(params) {
    let msg;
    const thisTimestamp = this.getScaledTimestamp(params.timestamp);
  ...
  respondReleaseStream(requestCommand, callback) {
    const streamName = requestCommand.objects[1] != null ? requestCommand.objects[1].value : undefined;
    logger.debug(`[rtmp] releaseStream: ${this.app}/${streamName}`);

    // TODO: Destroy stream here?

    const _result = createAMF0CommandMessage({
      chunkStreamID: 3,
      timestamp: 0,
      messageStreamID: 0,
      command: '_result',
  ...
  respondPublish(requestCommand, callback) {
    ...
    const urlInfo = url.parse(publishingName);
    ...
    const publishingType = requestCommand.objects[2] != null ? requestCommand.objects[2].value : undefined;
  ...
  getCodecConfigs(timestamp) {
    ...
    if (stream.isVideoStarted) {
      ...
      // video
      const spsLen = stream.spsNALUnit.length;
      const ppsLen = stream.ppsNALUnit.length;
      buf = new Buffer([
  ...
  respondSeek(requestCommand, callback) {
    ...
    if ((this.stream != null ? this.stream.type : undefined) === avstreams.STREAM_TYPE_RECORDED) {
      ...
      this.isPlaying = false;
      const _isPaused = this.stream.isPaused();
      if (!_isPaused) {
        this.stream.pause();
      }
  ...
  respondPause(requestCommand, callback) {
    const doPause = requestCommand.objects[1].value === true;
    const msec = requestCommand.objects[2].value;
  ...
  handleAMFCommandMessage(commandMessage, callback) {
    switch (commandMessage.command) {
      ...
      case 'play':
        var streamId = commandMessage.objects[1] != null ? commandMessage.objects[1].value : undefined;
...
class RTMPServer {
  ...
  dumpSessions() {
    ...
    if (rtmptSessionsCount > 0) {
      ...
      for (sessionID in rtmptSessions) {
        const rtmptSession = rtmptSessions[sessionID];
  ...
  teardownRTMPTClient(socket) {
    if (socket.rtmptClientID != null) {
      ...
      const tsession = rtmptSessions[socket.rtmptClientID];
      if (tsession != null) {
  ...
  sendVideoPacket(stream, nalUnits, pts, dts) {
    ...
    const timestamp = convertPTSToMilliseconds(dts);
    ...
    const buf = Buffer.concat(message);
  ...
  sendEOS(stream) {
    ...
    const lastTimestamp = stream.rtmpLastTimestamp != null ? stream.rtmpLastTimestamp : 0;

    const playComplete = createAMF0DataMessageParams({
      chunkStreamID: 4,
      timestamp: lastTimestamp,
      messageStreamID: 1,
      objects: [
...
`.replace(/\n/g, '\r\n');
  ...
  respondIdle(req, callback) {
    ...
    this.pendingResponses = [];
    const allBytes = Buffer.concat(bufs, totalLength);
